...
export default function AdminProductList() {
  ...
  const noImageCount = products.filter(p => isEnabled(p) && !p.imagen_url).length
  ...
  const filtered = visibleProducts.filter(p => {
    ...
    const matchNoImage = !showOnlyNoImage || (!p.imagen_url && isEnabled(p))
    return matchCat && matchSubcat && matchSearch && matchNoImage
  })
  ...
  async function toggleProduct(product) {
    const newValue = !isEnabled(product)
    ...
    try {
      ...
      setProducts(prev =>
        prev.map(p => p.id === product.id ? { ...p, disponible: newValue } : p)
      )
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Banner sin imagen */}
      {!loading && noImageCount > 0 && (
        <button
          onClick={() => setShowOnlyNoImage(v => !v)}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span>{noImageCount} producto{noImageCount !== 1 ? 's' : ''} activo{noImageCount !== 1 ? 's' : ''} sin imagen</span>
          </div>
          <span className="text-xs opacity-80">{showOnlyNoImage ? 'Ver todos' : 'Filtrar →'}</span>
        </button>
      )}
            ...
            return (
              <div key={product.id} className={enabled ? '' : 'opacity-50'}>
                ...
                <div className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  ...
                  {/* Toggle visibilidad por producto */}
                  <button
                    onClick={() => toggleProduct(product)}
                    disabled={togglingProductId === product.id}
                    title={enabled ? 'Ocultar producto' : 'Mostrar producto'}
