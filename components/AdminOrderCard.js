...
export default function AdminOrderCard({ order: initialOrder, adminPassword, onDelete }) {
  ...
  async function handleEnviarWhatsApp() {
    ...
    msg += `\n\nTe adjuntamos el PDF con los detalles.`
    if (paymentLink.trim()) {
      msg += `\n\n💳 *Enlace de pago:*\n${paymentLink.trim()}`
    } else {
        ...
        <div className="flex gap-2 flex-wrap">
          {order.status === 'pending' && (
            <>
              ...
              >
                Solo PDF {pdfSelected === null ? '(todos)' : pdfSelected.size === 0 ? '(ninguno)' : `(${pdfSelected.size})`}
              </button>
              <div className="w-full space-y-1.5">
                <input
                  ...
                  placeholder="Enlace de pago (opcional)"
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:border-gray-400"
                />
                <button
                  onClick={handleEnviarWhatsApp}
                  className="w-full py-2 bg-green-500 text-white text-xs font-semibold rounded-xl hover:bg-green-600 transition-colors"
                >
                  📎 PDF + Avisar cliente{paymentLink.trim() ? ' + Enlace de pago' : ''}
                </button>
              </div>
              <button
