...
export async function GET(req) {
  ...
  const { data, error } = await supabase
    .from('deudas_proveedor').select('*').order('fecha', { ascending: false })
...
export async function POST(req) {
  if (!isAdmin(req)) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
  const { proveedor, concepto, monto_total, monto_pagado, fecha, fecha_vencimiento, notas } = await req.json()
  if (!proveedor || !monto_total) return NextResponse.json({ error: 'Proveedor y monto requeridos' }, { status: 400 })
  ...
  const { data, error } = await supabase
    ...
    .insert({
      proveedor, concepto, monto_total, monto_pagado: pagado,
...
export async function PUT(req) {
  ...
  const supabase = getSupabase()
  const { data, error } = await supabase.from('deudas_proveedor').update(fields).eq('id', id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
