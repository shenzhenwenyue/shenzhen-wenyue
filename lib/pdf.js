...
export async function generarCotizacionPDF(order, replacements = {}, shippingCost = 0) {
  ...
  // Construir lista unificada de filas: normales + sugeridas mezcladas
  const allRows = []
  order.items.forEach((item, idx) => {
    ...
    if (item.confirmed === true) {
      allRows.push({
        ...
        qty: item.available_qty ?? item.qty,
        unit_price: item.unit_price,
        imagen_url: item.imagen_url,
      })
      // Si hay sugeridos adicionales (disponibilidad parcial), también los agrego
      sugs.forEach(sug => {
        allRows.push({
          tipo: 'replacement',
          nombre: sug.nombre,
  ...
  // Items agotados sin ningún sugerido
  ...
  if (allRows.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: allRows.map(row => {
        const productCell = row.tipo === 'replacement'
          ? row.nombre + '\n→ Suggested · ' + row.originalName + ' out of stock'
          : row.nombre
        return [
          '',
          productCell,
    ...
    const subtotal = allRows.reduce((s, row) => s + row.qty * row.unit_price, 0)
    const afterTable = doc.lastAutoTable.finalY + 8
  ...
  // Items agotados sin sugerencia (solo nota al pie)
  if (sinReemplazo.length > 0) {
    if (startY > 265) { doc.addPage(); startY = 20 }
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(9)
    doc.setTextColor(150, 150, 150)
    doc.text('Out of stock (no replacement available): ' + sinReemplazo.map(i => i.nombre).join(', '), 20, startY, { maxWidth: 170 })
    startY += 10
  }
