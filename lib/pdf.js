...
export async function generarCotizacionPDF(order, replacements = {}, shippingCost = 0) {
  ...
  if (available.length > 0) {
    ...
    const subtotalReemplazos = repEntries.reduce((s, { original, replacement }) => s + (original.available_qty || original.qty) * (replacement.unit_price || 0), 0)
    ...
    if (repEntries.length > 0) {
      doc.setFontSize(8)
      doc.setTextColor(150, 150, 150)
      doc.text('(incluye alternativas sugeridas)', 190, afterTable + 5, { align: 'right' })
      doc.setFontSize(10)
      doc.setTextColor(80, 80, 80)
    }
    const offsetEnvio = repEntries.length > 0 ? 7 : 0
    if (shippingCost > 0) {
      doc.text('Envío estimado: $' + shippingCost.toFixed(2), 190, afterTable + 7 + offsetEnvio, { align: 'right' })
    }
    ...
    autoTable(doc, {
      ...
      didDrawCell: (data) => {
        if (data.section === 'body') {
          const entry = repEntries[data.row.index]
          if (!entry) return
          if (data.column.index === 0) {
            const imgData = entry.replacement?.imagen_url && imageMap[entry.replacement.imagen_url]
            if (imgData) {
